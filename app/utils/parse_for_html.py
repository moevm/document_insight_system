...
def find_tasks_on_slides_feedback(slides_with_tasks):
    return (
        format_header('Всего задач: {}'.format(slides_with_tasks.get('count')), tag.br),
...
def tasks_conclusions_feedback(results):
    return (
        format_header('Соответствует на {}%'.format(results[0]), tag.br),
...
def name_of_image_check_results(result_str, all_captions):
    return (
        format_header(result_str)
        + 'Список всех обнаруженных подписей:'
