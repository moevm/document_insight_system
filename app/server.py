...
def run_task():
    ...
    no_group = 'No group given'
    no_comments = 'No comments given'
    file = request.files.get("file")
    file_type = request.form.get('file_type', 'pres')
    optional = request.form.get('fullname', no_name) if request.form.get('fullname') != '' else no_name
    optional += ', ' + (request.form.get('group', no_group) if request.form.get('group') != '' else no_group)
    optional += ': ' + (request.form.get('comments', no_comments) if  request.form.get('comments') != '' else no_comments)
    parser1 = optional.split(', ')
    fullname = parser1[0]
    parser2 = parser1[1].split(': ')
    group = parser2[0]
    comments = parser2[1]
    if fullname == no_name and group == no_group and comments == no_comments:
        optional = 'No additional info given'
    if not file:
