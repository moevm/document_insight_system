...
class MdUploader(DocumentUploader):
    ...
    def page_counter(
        self,
    ):  # we need this just to find a last page and make link to the literature in banned_words_in_literature
    ...
    def chapters_to_str(self):
        if self.str_chapters:
            return self.str_chapters
        result = []
    ...
    def build_chapter_tree(self, chapters):
        if self.tree_chapters:
            return self.tree_chapters
        tree = []
        stack = [tree]
        ...
        for chapter in chapters:
            ...
            new_chapter = {'name': chapter['name'], 'text': chapter['text'], 'children': []}
