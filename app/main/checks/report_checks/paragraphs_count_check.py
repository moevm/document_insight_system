...
class ReportParagraphsCountCheck(BaseReportCriterion):
    ...
    def __init__(
        self,
        file_info,
        min_paragraphs_in_unnumbered_section=2,
        min_paragraphs_in_section=5,
        min_paragraphs_in_subsection=5,
        min_paragraphs_in_subsubsection=1,
        skip_sections=None,
    ):
    ...
    def check(self):
        ...
        for obj in self.paragraphs_count:
            if obj["paragraphs"] < obj["min"]:
                result = False
                result_str += (
                    f'Раздел "{obj["name"]}" содержит {obj["paragraphs"]} абзацев '
    ...
    def find_paragraphs_count(self):
        ...
        while i < len(self.headers):
            ...
            if self.headers[i]["style"] == self.heading_styles['any_header'][0] and not re.search(
                ...
                while j < len(self.headers) and self.headers[j]["style"] == self.heading_styles['second_header'][1]:
                    ...
                    while k < len(self.headers) and self.headers[k]["style"] == self.heading_styles['second_header'][2]:
                        count_lists_subsubsection, ignored_paragraphs = self.find_lists_and_captions(
                            self.headers[k]["child"]
                        )
                        subsubsection_count = (
                            len(self.headers[k]["child"]) - ignored_paragraphs + count_lists_subsubsection
                        )
