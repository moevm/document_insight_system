...
class TableReferences(BaseReportCriterion):
    ...
    def check(self):
        ...
        if self.file_type['report_type'] == 'VKR':
            ...
            if not number_of_tables:
                return answer(False, f'Не найдено ни одной таблицы.<br><br>Убедитесь, что для подписи таблиц был использован стиль {self.table_style}, а таблица подписана "Таблица <Номер таблицы> -- <Название таблицы>".')
        else:
        ...
        if len(references.symmetric_difference(all_numbers)) == 0:
            ...
            if len(all_numbers.difference(references)) == 0:
                ...
                result_str += f'Упомянуты несуществующие таблицы: {", ".join(str(num) for num in sorted(references))} <br> Номера таблиц: {", ".join(num for num in sorted(all_numbers))}<br><br>'
            else:
                extras = references - all_numbers
                unnamed = all_numbers - references
                result_str += f'Упомянуты несуществующие таблицы: {", ".join(str(num) for num in sorted(extras))} <br> А также упомянуты не все таблицы: {", ".join(str(num) for num in sorted(unnamed))} <br> Номера таблиц: {", ".join(num for num in sorted(all_numbers))}<br><br>'
        else:
            all_numbers -= references
            result_str = f'Упомянуты не все таблицы.<br>Список таблиц без упоминания: {", ".join(str(num) for num in sorted(all_numbers))} <br> Номера таблиц: {", ".join(num for num in sorted(all_numbers))}<br><br>'
        result_str += f'''
    ...
    def search_references(self):
        ...
        for i in range(0, self.last_child_number):
            detected_references = re.findall(r'таблиц[аеы][\d .]+', self.file.paragraphs[i].paragraph_text)
            if detected_references:
                for reference in detected_references:
                    for one_part in re.split(r'таблиц[аеы]| ', reference):
