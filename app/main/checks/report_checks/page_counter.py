...
class ReportPageCounter(BaseReportCriterion):
    ...
    def check(self):
        ...
        if count >= self.number[0] and (self.number[1] is None or count <= self.number[1]):
            return answer(
                True,
                f"Пройдена! В отчете {count} стр не считая Приложения. <br>"
                f"Последняя учтенная страница {self.format_page_link([self.file.page_count])}.",
            )
        if self.number[1]:
            result_str += (
                f'Неверное количество страниц в файле: должно быть [{self.number[0]}, {self.number[1]}] '
            ...
            result_str += (
                f'Неверное количество страниц в файле: должно быть не менее {self.number[0]} стр. '
        ...
        result_str += (
            '''
                    ...
                    <ul>
                        <li>'''
            + 'Убедитесь, что заголовок "ПРИЛОЖЕНИЕ А" начинается с первой строки новой страницы PDF-файла;'
            + '''</li>
                        <li>Убедитесь, что для заголовок "ПРИЛОЖЕНИЕ А" имеет стиль '''
            + '"Заголовок 2,ВКР_Заголовок 2";</li>'
            + '''
                        <li>Убедитесь, что PDF-файл корректно отображает оформление.</li>
                    </ul>
                    '''
        )
        result_str += "Если сгенерированный PDF-файл имеет проблемы с оформлением, попробуйте загрузить свой PDF."
