...
class ReportDecimalPlacesCheck(BaseReportCriterion):
    ...
    def check(self):
        ...
        total_violations, detected_pages = self.checker.find_violations_in_texts(
            self.file.pdf_file.get_text_on_page(end_page=literature_page).items()
        )
        result_str, result_score = self.checker.get_result_msg_and_score(
            total_violations, detected_pages, self.format_page_link
        )
