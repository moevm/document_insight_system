...
class ImageReferences(BaseReportCriterion):
    ...
    def check(self):
        ...
        if count_file_image_object and not number_of_images:
            return answer(
                False,
                f'В отчёте найдено {count_file_image_object} рисунков, но не найдено ни одной подписи рисунка.<br><br>'
            ...
            return answer(
                True,
                f'Не найдено ни одного рисунка.<br><br> Если в вашей работе присутствуют рисунки, убедитесь, '
                f'что для их подписи был использован стиль {self.image_style}, и формат: '
                f'"Рисунок <Номер рисунка> — <Название рисунка>".',
            )
