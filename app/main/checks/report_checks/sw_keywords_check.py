...
class SWKeywordsCheck(BaseReportCriterion):
    ...
    _description = (
        'Раздел идет сразу после названия работы и содержит не менее трех ключевых слов. Слова упоминаются в тексте'
    )
    ...
    def check(self):
        ...
        if self.lemme_list:
            return answer(
                False, f"Не пройдена! В тексте не найдены следующие ключевые слова: «{'», «'.join(self.lemme_list)}»"
            )
        else:
