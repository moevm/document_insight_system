...
TEMPLATE_TYPES = {
    'GFLab': "Проверка соответствия названия файла шаблону: Отчет_ЛР[1-9]{1}(_[А-Я]{1}[а-яё]*)+, например - 'Отчет_ЛР1_Иванова_Петрова'",  # noqa: E501
    'CSLab': "Проверка соответствия названия файла шаблону ([номер_студ_билета][фамилия]_ОТЧЕТ_ЛР[номер_ЛР]): [0-9]{6}([А-ЯЁ]+)_ОТЧЕТ_ЛР[1-3]{1}, например - '534111ИВАНОВ_ОТЧЕТ_ЛР1'",  # noqa: E501
    'VKR': f"Проверка соответствия названия файла шаблону: {CUR_YEAR}"
    + r"ВКР[0-9]{6}([А-ЯЁ]+)"
    + f', например - "{CUR_YEAR}ВКР111111ИВАНОВ"',
}
...
class ReportTemplateNameCheck(BasePresCriterion):
    ...
    def check(self):
        ...
        return answer(False, f'Название файла "<i>{self.filename}</i>" не соответствует шаблону: {self.reg}')
