    ...
    def check(self):
        try:           
            ...
            if not bool_end_content_find:
                return answer(False, "Не найдено заголовка 'ОПРЕДЕЛЕНИЯ, ОБОЗНАЧЕНИЯ И СОКРАЩЕНИЯ' (заголовок второго уровня), который должен идти сразу после раздела 'Содержание'")

            if failed_str:
                result_str = f"Найдено {len(failed_str)} заголовков 3 уровня и выше:<br>" \
