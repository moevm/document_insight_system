...
def get_filtered_module_members(module):
    return filter(
        lambda member: inspect.isclass(member[1]) and issubclass(member[1], BaseCriterion), inspect.getmembers(module)
    )
...
CRITERIA_INFO = {
    'pres': {
        cls.id: CriteriaInfo({'class': cls, 'name': name, 'label': cls.label})
        for name, cls in get_filtered_module_members(presentation_checks)
    },
    'report': {
        cls.id: CriteriaInfo({'class': cls, 'name': name, 'label': cls.label})
        for name, cls in get_filtered_module_members(report_checks)
    },
}
