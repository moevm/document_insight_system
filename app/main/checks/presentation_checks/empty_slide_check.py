    ...
    def check(self):
        ...
        if self.file.presentation_name.endswith('.ppt') or self.file.presentation_name.endswith('.pptx'):
            ...
            if empty_pages and pages_with_title:
                result_str += format_header(
                    'Не пройдена! Обнаружены пустые слайды: {}, также обнаружены слайды, в которых присутствует только заголовок: {}'.format(  # noqa: E501
            ...
            if empty_pages:
                result_str += format_header(
                    'Не пройдена! Обнаружены пустые слайды или слайды, в которых присутствует только заголовок: {}'.format(  # noqa: E501
                        ', '.join(self.format_page_link(empty_pages))
                    )
                )
