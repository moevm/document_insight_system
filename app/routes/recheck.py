...
from app.tasks import convert_check_file_to_pdf, create_task
...
def recheck_main(check_id):
    ...
    task_chain = chain(convert_check_file_to_pdf.s(check.pack(to_str=True), filepath, rewrite=True), create_task.s())
