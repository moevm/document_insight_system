...
class Version:
    ...
    @classmethod
    def to_dict(cls):
        return dict(version=cls.VERSION_NAME, changes=cls.CHANGES)
...
class Version11(Version):
    VERSION_NAME = '1.1'
    CHANGES = (
        "В коллекцию checks добавлены поля filename, user и score. Для перехода с версии 1.0 необходимо заполнить поля"
    )

    @classmethod
    def update_database(cls, collections, prev_version):
        if prev_version == Version10.VERSION_NAME:
            collections['checks'].update({}, {'$set': {'filename': '_.pptx', 'user': 'moevm', 'score': -1}})
        else:
...
class Version20(Version):
    VERSION_NAME = '2.0'
    CHANGES = (
        "Изменен подход к версионированию БД: исправляются минусы предыдущих версий "
        ...
        if prev_version in (Version10.VERSION_NAME, Version11.VERSION_NAME):
            ...
            for presentation in collections['presentations'].find({}):
                ...
                for check_id in presentation["checks"]:
                    collections['checks'].update({"_id": check_id}, {'$set': {'filename': filename, 'user': user}})
            ...
            collections['checks'].update({'filename': {"$exists": 0}}, {'$set': {'filename': '_.pptx'}}, multi=True)
            collections['checks'].update({'user': {"$exists": 0}}, {'$set': {'user': 'moevm'}}, multi=True)

            # calc score for all checks w/score=-1
            for check in collections['checks'].find({'score': -1}):
                score = Check(check).calc_score()
                collections['checks'].update({'_id': check['_id']}, {'$set': {'score': score}})

            # update criteria.slides_number
            for user in collections['users'].find():
                new_criteria = {12: [10, 12], 15: [13, 15]}[user['criteria']['slides_number']]
                collections['users'].update({'_id': user['_id']}, {'$set': {'criteria.slides_number': new_criteria}})
    ...
    def update_database(cls, collections, prev_version):
        if prev_version in (Version10.VERSION_NAME, Version11.VERSION_NAME, Version20.VERSION_NAME):
            ...
            for check in collections['checks'].find({}):
                ...
                collections['checks'].update({'_id': check['_id']}, {'$set': upd_check})

            for user in collections['users'].find():
                criteria_dt = Check(user['criteria']).enabled_checks
                upd_criteria = {k: False if v == -1 else True for k, v in criteria_dt.items()}
                upd_criteria['slides_number'] = (
                    {"sld_num": criteria_dt['slides_number'], "detect_additional": True}
                    if upd_criteria['slides_number']
                    else False
                )
                collections['users'].update({'_id': user['_id']}, {'$set': {'criteria': upd_criteria}})
...
class Version22(Version):
    VERSION_NAME = '2.2'
    CHANGES = (
        'Changes criteria field in user to dict with enabled values\n'
    ...
    def update_database(cls, collections, prev_version):
        if prev_version in (
            Version10.VERSION_NAME,
            Version11.VERSION_NAME,
            Version20.VERSION_NAME,
            Version21.VERSION_NAME,
        ):
            criteria_keys = (
                'slides_number',
                'slides_enum',
            ...
            check_info = ('score', 'filename', 'conv_pdf_fs_id', 'user', 'is_passbacked', 'lms_passback_time')
            ...
            for user in collections['users'].find():
                ...
                collections['users'].update_one({'_id': user['_id']}, {'$set': {'criteria': reorder_criteria}})
            collections['users'].update_many({}, {"$unset": unset_fields})

            pipeline = [
                {
                    '$project': {
                        'enabled_checks': dict((f'{k}', f'${k}') for k in criteria_keys),
...
class Version30(Version):
    ...
    def map(key, value):
        if key == 'slides_number':
            result = {'id': key, 'name': "Количество основных слайдов"}
        if key == 'slides_enum':
            result = {'id': key, 'name': "Нумерация слайдов"}
        if key == 'slides_headers':
            result = {'id': key, 'name': "Заголовки слайдов присутствуют и занимают не более двух строк"}
        if key == 'goals_slide':
            result = {'id': 'find_slides', 'name': "Слайд 'Цель и задачи'"}
        if key == 'probe_slide':
            result = {'id': 'find_slides', 'name': "Слайд 'Апробация работы'"}
        if key == 'actual_slide':
            result = {'id': 'find_on_slide', 'name': "Слайд с описанием актуальности работы"}
        if key == 'conclusion_slide':
            result = {'id': 'find_slides', 'name': "Слайд с заключением"}
        if key == 'slide_every_task':
            result = {'id': key, 'name': "Наличие слайдов, посвященных задачам"}
        ...
        if key == 'conclusion_along':
            result = {'id': 'future_dev', 'name': "Наличие направлений дальнейшего развития"}
        if key == 'template_name':
            result = {'id': key, 'name': 'Соответствие названия файла шаблону'}
...
class Version31(Version):
    VERSION_NAME = '3.1'
    CHANGES = (
        'Изменена структура наборов критериев - file_type: "pres" => {"type": "pres"}. Обновлены наборы и пользователи'
    )

    @classmethod
    def update_database(cls, collections, prev_version):
        if prev_version in (Version30.VERSION_NAME,):
            packs = {'pres': {'type': 'pres'}, 'report': {'type': 'report', 'report_type': 'LR'}}
            # set new file_type for pack depends on old type
            for pack_name in packs:
                collections['criteria_pack'].update_many(
                    {'file_type': pack_name}, {'$set': {'file_type': packs[pack_name]}}
                )
...
class Version40(Version):
    VERSION_NAME = '4.0'
    CHANGES = (
        "Произведено переименование коллекции 'presentations' -> 'files' для загруженных файлов "
        "(и в модели пользователя). Требуется ручное переименование БД 'pres-parser-db' -> 'dis-db'!"
    )
